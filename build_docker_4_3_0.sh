...
#   ./build_docker_4_3_0.sh            -> local, native arch
#   ./build_docker_4_3_0.sh --push     -> multi-arch manifest to Docker Hub
#
...
if [ "${1:-}" = "--push" ]; then
    exec docker buildx bake --builder "$BUILDER" v4_3_0 --push
fi
exec docker buildx bake --builder "$BUILDER" v4_3_0_local --load
